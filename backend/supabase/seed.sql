-- ⚠️ DEVELOPMENT ONLY — DO NOT RUN ON PRODUCTION ⚠️
-- Synthetic demo seed. NIK values are hashes/placeholders only; never use real personal data.
-- Production databases must stay clean: run this ONLY on local/dev environments
-- (set DEMO_SEED=false / APP_ENV=production guard is enforced by team policy).
