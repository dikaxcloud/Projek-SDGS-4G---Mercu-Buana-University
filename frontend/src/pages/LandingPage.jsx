...
export function LandingPage() {
  ...
  return <>
    <main id="main-content">
      ...
      <section className="section" id="nakes"><div className="container"><div className="section-head"><div><div className="eyebrow">Tim kesehatan desa</div><h2 className="display">Ada yang siap membantu.</h2></div><Link to="/tim-kesehatan" className="btn btn-soft">Lihat semua <ChevronRight size={16} /></Link></div>{workers.length === 0 ? <p className="muted-text">Belum ada data tenaga kesehatan.</p> : <div className="team-grid">{workers.map((worker) => <div className="team-card" key={worker.name}>{worker.avatar_url ? <img src={worker.avatar_url} alt={worker.name} width={40} height={40} loading="lazy" decoding="async" style={{width:40,height:40,borderRadius:'50%',objectFit:'cover', flex:'0 0 40px'}} onError={e=>{e.currentTarget.style.display='none'; e.currentTarget.nextSibling.style.display='grid'}}/> : null}<div className="avatar" style={{display: worker.avatar_url?'none':'grid'}}>{worker.initials}</div><div style={{flex:1, minWidth:0}}><h3>{worker.name}</h3><p>{worker.role} · {worker.specialty}</p><span className="online" style={{ color: worker.online ? '#15803d' : 'var(--muted)' }}>{worker.online ? 'Sedang online' : 'Tidak sedang online'}</span></div><Link to={`/tim-kesehatan/${worker.health_worker_id || worker.user_id || ''}`} className="btn btn-ghost" style={{marginLeft:'auto', minHeight:34, padding:'0 10px', fontSize:12, borderColor:'var(--line)'}}>Lihat profil</Link></div>)}</div>}</div></section>
