-- Migration: cascade cleanup saat hapus akun dari halaman admin
-- Masalah: health_workers.user_id punya FK "on delete set null" ke profiles,
--          jadi saat akun dihapus lewat Admin & Akses, baris direktori nakes
--          TIDAK ikut terhapus (user_id jadi NULL, is_active masih true)
--          -> akun yang sudah dihapus masih muncul di halaman Nakes.
-- Solusi: admin_delete_user & admin_delete_citizen(with_account) ikut
--         menghapus baris health_workers yang terhubung sebelum akun dihapus.
...
begin
  ...
  -- Bersihkan data yang terhubung sebelum akun dihapus
  ...
  return jsonb_build_object('status', 'deleted', 'health_worker_rows_deleted', v_dir_deleted);
end;
$$;

-- Sama untuk hapus warga beserta akun Google-nya
...
returns jsonb language plpgsql security definer set search_path = public as $$
declare
  v_name text; v_records integer; v_linked_user uuid; v_linked_role text;
  v_account_deleted boolean := false; v_dir_deleted integer := 0;
begin
  perform public.admin_guard();
  ...
  if p_with_account and v_linked_user is not null then
    ...
    -- Bersihkan data yang terhubung sebelum akun dihapus
    delete from public.health_workers where user_id = v_linked_user;
    get diagnostics v_dir_deleted = row_count;
  ...
  insert into public.audit_logs (actor_user_id, action, entity, entity_id, metadata)
    values (auth.uid(), 'delete', 'citizen', p_citizen_id, jsonb_build_object(
      'full_name', v_name,
      'health_records_deleted', v_records,
      'google_account_deleted', v_account_deleted,
      'health_worker_rows_deleted', v_dir_deleted));
